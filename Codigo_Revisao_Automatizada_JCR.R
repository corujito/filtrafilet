...
selecaoDocumentos <- function(arquivo, jcr.min, ano.min, cit.ano, porc.pareto) {
            ...
            ##dados2 <- read.csv(dados2, sep = ",", stringsAsFactors = FALSE)
            ...
            ## Base com JCR ##
            jcr <- read.csv('/Users/renatapelissari/Downloads/JournalHomeGrid.csv', header = F, sep = ",", stringsAsFactors = FALSE)
            jcr <- jcr[-1,]
            jcr[,2] <- toupper(jcr$V2) #Deixando o título dos Journals todo com letra maiuscula, pois será utilizado como chave
           ...
           # print(nrow(filtro.artigos.antigos)) #596 linhas
...
selecaoDocumentos("/Users/renatapelissari/Downloads/savedrecs (2).txt", 1.5, 2, 0, 70) 
